...
def process_file(user_id, new_filename):
    ...
    return send_file(result_path, as_attachment=True, mimetype='application/octet-stream')
...
def verify_result():
    ...
    if received_idx == stored_idx:
        return jsonify({"message": "Identical Verified"}), 200
    elif received_idx == -1:
        return jsonify({"message": "Not identical Verified"}), 200
    else:
        return jsonify({"error": "Untrusted Response"}), 400
